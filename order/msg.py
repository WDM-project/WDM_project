...
import json
import requests
import transbase, dtmimp, barrier, utils
...
class Msg(object):
    ...
    def do_and_submit_db(self, query_prepared, conn, busi_call):
        return self.do_and_submit(
            query_prepared, lambda barrier: barrier.call(conn, busi_call)
        )

    def do_and_submit(self, query_prepared, busi_call):
        bb = barrier.BranchBarrier(
            self.trans_base.trans_type, self.trans_base.gid, "00", "msg"
        )
        ...
        if errb is not None and not isinstance(errb, utils.DTMFailureError):
            try:
                ...
                err = e
        if isinstance(errb, utils.DTMFailureError) or isinstance(
            err, utils.DTMFailureError
        ):
            dtmimp.trans_call_dtm(
                self.dtm,
                self.trans_base.__dict__,
